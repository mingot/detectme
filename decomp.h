...
void lasso2(const Matrix<T>& X, const Matrix<T>& D, SpMatrix<T>& spalpha,
      ...
      const int numThreads = -1, Matrix<T>* path = NULL);
...
void coreORMP(Vector<T>& scores, Vector<T>& norm, Vector<T>& tmp, Matrix<T>& Un,
   ...
   for (j = 0; j<L; ++j) {
      ...
      scores.sqr(Rdn);
      scores.div(norm);
      // for (int k = 0; k<K; ++k) if (norm[k] < EPSILON) scores[k]=T();
...
void lasso(const Matrix<T>& X, const Matrix<T>& D, SpMatrix<T>& spalpha, 
   ...
   lasso(X,G,DtX,spalpha,L,lambda,mode,pos,ols,numThreads,path);
}
...
void coreLARS(Vector<T>& Rdnv, Vector<T>& Xdnv, Vector<T>& Av,
   ...
   for (j = 0; j<L; ++j) {
      ...
      if (pos) {
         for (int k = 0; k<K; ++k) {
            T diff = a-A[k];
            work[k]= diff <= 1e-10 ? INFINITY : (Cmax-Rdn[k])/diff;
         }
         for (int k = 0; k<=j; ++k) {
            work[ind[k]]=INFINITY; 
         }
       //  workm.print("workm");
         ...
         for (int k = 0; k<K; ++k) {
            ...
            if (!work[index]) {
               const T diff1=a-A[k];
               work[index]= diff1 <= 1e-10 ? INFINITY : (Cmax-Rdn[k])/diff1;
               const T diff2=a+A[k];
               work[index+1]=diff2 <= 1e-10 ? INFINITY : (Cmax+Rdn[k])/diff2;
            }
         }
      ...
      if (j > 0) {
         vDiv<T>(j,coeffs,u,work);
         cblas_scal<T>(j,-T(1.0),work,1);
         /// voir pour petites valeurs
         for (int k=0; k<j; ++k) 
            if (work[k] <0) work[k]=INFINITY;
         minBasis=cblas_iamin<T>(j,work,1);
      ...
      if ((j == L-1) || 
            (mode == PENALTY && (thrs - constraint < 1e-10)) ||
            (mode == L1COEFFS && (thrs - constraint > -1e-10)) || 
            (newAtom && mode == L2ERROR && (normX - constraint < 1e-10)) ||
            (normX < 1e-10) ||
            (iter > 4*L && newAtom)) break;
...
void lasso2(const Matrix<T>& X, const Matrix<T>& D, SpMatrix<T>& spalpha, 
      int L, const T constraint, const T lambda2, constraint_type mode, const bool pos,
      const int numThreads, Matrix<T>* path) {
   ProdMatrix<T> G(D,X.n() > 10 && D.n() < 50000);
   ProdMatrix<T> DtX(D,X,false);
   G.addDiag(MAX(lambda2,1e-10));
   lasso2(X,G,DtX,spalpha,L,constraint,mode,pos,numThreads,path);
}
...
void coreLARS2(Vector<T>& DtR, const AbstractMatrix<T>& G,
   ...
   for (i = 0; i<L; ++i) {
     ...
     for (int j = 0; j<=i; ++j) {
      ...
      for (int j = 0; j<K; ++j) {
         pr_work[j] = ((pr_work[j] < INFINITY) && (pr_work[j] > T(-0.99999999))) ? (pr_DtR[j]+current_correlation)/(T(1.0)+pr_work[j]) : INFINITY;
      }
 //     work.print("work");
      for (int j = 0; j<K; ++j) {
         pr_work[j+K] = ((pr_work[j+K] < INFINITY) && (pr_work[j+K] < T(0.99999999))) ? (current_correlation-pr_DtR[j])/(T(1.0)-pr_work[j+K]) : INFINITY;
      }
 //     work.print("work");
...
//      PRINT_F(thrs)
      ...
      if (iter > 4*L || abs(step) < 1e-10 ||
            step == step_max2 || (normX < 1e-11) ||
            (i == (L-1)) ||
            (mode == L2ERROR && normX - constraint < 1e-11) ||
            (mode == L1COEFFS && (constraint-thrs < 1e-11))) {
         break;
      }
   }
}
...
void coreLARS2W(Vector<T>& DtR, AbstractMatrix<T>& G,
   ...
   for (i = 0; i<L; ++i) {
    ...
    /*  for (int j = 0; j<K; ++j) {
         pr_work[j]=(pr_DtR[j]+current_correlation)/(1+pr_work[j]);
      }
      for (int j = 0; j<K; ++j) {
         pr_work[j+K]=(current_correlation-pr_DtR[j])/(1-pr_work[j+K]);
      }
      for (int j = 0; j<=i; ++j) {
         pr_work[pr_ind[j]]=INFINITY;
         pr_work[pr_ind[j]+K]=INFINITY;
      ...
      for (int j = 0; j<2*K; ++j)
         if (pr_work[j] < 0) pr_work[j]=INFINITY;*/
      if (pos) {
         for (int j = 0; j<K; ++j) {
            pr_work[j]=INFINITY;
         }
